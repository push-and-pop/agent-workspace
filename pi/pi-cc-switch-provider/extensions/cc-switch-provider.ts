...
// Auth headers stay controlled by cc-switch routing: the primary route and the
// independent-summary route may use different credentials, so options.headers
// (which carries pi's `before_provider_headers` pipeline output, e.g. the
// spoof-headers plugin) must never overwrite them.
const CC_SWITCH_PROTECTED_HEADERS = new Set(["authorization", "x-api-key"]);

function mergeTransformedHeaders(target: Record<string, string>, source?: Record<string, string | null>): void {
	if (!source) return;
	for (const [key, value] of Object.entries(source)) {
		if (CC_SWITCH_PROTECTED_HEADERS.has(key.toLowerCase())) continue;
...
function streamCcSwitchCodexResponses(
	...
	(async () => {
		...
		try {
			...
			mergeTransformedHeaders(headers, options?.headers);
